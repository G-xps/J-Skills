...
- `groupId` uses reverse-domain style by default, for example `com.example`.
- `artifactId` uses lowercase kebab-case, for example `j-skill-demo`.
- `base package` is derived from `groupId` and the project name, for example `com.example.jskilldemo`.
